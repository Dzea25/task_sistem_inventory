                    ...
                    <div class="card-body p-0">
                        <!-- Nested Row within Card Body -->
                                <div class="p-5">
                                    ...
                                    <?php 
                                        $id = $_GET['id']; 
                                        $qEditBarangKeluar = "select * from barangkeluar where id='$id'";
                                        $data = $myConn->query($qEditBarangKeluar);             
                                        while($d = $data->fetch_array()){
                                    ?>
                                    <form action="barang_masuk_act.php" method="post">
                                        <div class="form-group">
                                            <label><b>Tanggal Keluar Barang</b></label>
                                            <input type="hidden" name="id" value="<?php echo $d['id'] ?>">
                                            <input type="date" class="form-control form-control-user"
                                                id="Tanggal" name="tanggal" placeholder="Silahkan masukkan tanggal..." 
                                                required="required" autocomplete="off" value="<?php echo $d['tgl_keluar'] ?>>
                                        </div>
                                        <div class="form-group">
                                            <label><b>Jumlah Barang Keluar</b></label>
